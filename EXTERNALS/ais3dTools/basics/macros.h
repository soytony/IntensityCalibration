...
#ifdef WIN32
  #include <windows.h>
  #define ais_sleep(x) Sleep(1000*(x))
  #define ais_ms_sleep(x) Sleep(x)
#else
  #define ais_sleep(x) sleep(x)
  #define ais_ms_sleep(x) usleep(1000*(x))  
#endif
...
// MSVC on Windows
  ...
  __inline bool isnan(real x) { return bool(_isnan(x)!=0); }
  template <typename real>
  __inline bool isfinite(real x) { return bool(_finite(x)!=0); }
  template <typename real>
  __inline bool isinf(real x) { return !isfinite(x) && !isnan(x); }
  
  #define INFINITY std::numeric_limits<float>::infinity()
  #define NAN std::numeric_limits<float>::quiet_NaN()
  
  __inline double round(double number) { return (number < 0.0  ? ceil(number - 0.5)  : floor(number + 0.5)); }
  __inline float  round(float number)  { return (number < 0.0f ? ceil(number - 0.5f) : floor(number + 0.5f));}
  
  template <typename real>
  __inline long int lrint(real x) { return static_cast<long int>(round(x)); }
  __inline long int lrintf(float x) { return lrint(x); }
